...
export async function generateMetadata({ params }: BasicSlugPageProps) {
  ...
  const data = await fetchDato(NetworkDocument, { locale: siteLocale, slug }, false);
...
export default async function Page({ params }: BasicSlugPageProps) {
  ...
  const siteLocale = locale as SiteLocale;
  const data = await fetchDato(NetworkDocument, { locale: siteLocale, slug }, isEnabled);
  const heroData: any = {
  ...
  const slugData = extractSlugData(data.network);
  return (
