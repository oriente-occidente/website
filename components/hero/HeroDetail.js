    ...
    <header className="border-t md:border-none border-gray pb-10 md:py-0 md:grid md:grid-cols-4 md:gap-4 xl:container xl:mx-auto">
        ...
        isFestival == true ?
          <div className="border-b border-gray py-3 md:border-none md:pt-4 xl:pt-8">
            <div className="container">
              <div className="uppercase text-black text-xxs font-semibold tracking-wider md:pb-2">Festival</div>
                {
                  dateEvento != null ?
                    <>
                      {dateEvento.map((date) => (
      ...
      <div className="relative md:col-span-3 md:border-l md:border-color-gray xl:pt-4">
        <div className="px-4 md:px-12">
          <h1 className="text-base uppercase pt-4 pb-1">
            {titleHero}
          </h1>
          <h2 class="text-base uppercase font-semibold">
            {descriptionHero}
          </h2>
        </div>
          {
            paymentSettings != null ?
                <>
                  {paymentSettings.map((item) => (
                    <div key={item.id} className="px-4 md:px-12 lg:flex justify-between items-center">
