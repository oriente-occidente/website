...
export function groupDatesByDay(dates, locale) {
  ...
  return Object.keys(groups).map((day) => {
    const date = dayjs(day).locale(locale).format('MMM DD YYYY');
    const times = groups[day].reduce((daily, d) => {
      if (d.isDaily) {
        daily.push(dayjs(d.startTime).locale(locale).format('HH:mm'));
      }
      return daily;
    }, []);
    if (times.length > 0) {
