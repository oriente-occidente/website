...
query projects($locale: SiteLocale) {
  projects: allProjects(locale: $locale, orderBy: startDate_DESC, filter:{slug: {neq: null}}) {
    ${sharedPreviewBlock}
    startDate
    category {
      id
...
query workshops($locale: SiteLocale) {
  workshops: allWorkshops(locale: $locale, filter: {slug: {neq: null},isWorkshop: {eq: true}}, orderBy: startDate_DESC, first: 100) {
    ${sharedPreviewBlock}
    authors
    category {
      id
...
query languages($locale: SiteLocale) {
  languages: allWorkshops(locale: $locale, filter: {slug: {neq: null},isWorkshop: {eq: false}}, orderBy: startDate_DESC, first: 100) {
    ${sharedPreviewBlock}
    category {
      id
      slug
