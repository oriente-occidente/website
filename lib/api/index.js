...
const queryRoutesByParams(params, locale, preview){
  //la magia avviene qui, dagli slug e la lingua devo tirare fuore l'equivalente del file di rotte
};
...
const matchRouteParams = (routes, locale, params) => {
...
export const doQuery = async (routes, locale, params, preview = false) => {
  const queries = [];
  queries.push({ query: 'site', values: { locale } });

  const pageInfo = matchRouteParams(routes, locale, params);
  if (pageInfo) {
...
export const doQueryItem = async (routes, locale, itemId) => {
  let myQuery = null;
  let pageInfo = null;
  if (itemId) {
